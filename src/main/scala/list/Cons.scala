...
case class Cons (val head:Int, val tail:IntList) extends IntList{
 ...
 def insertionSortI:IntList= 
  ...
  def map(mapFun:(Int)=>Int): IntList = Cons(mapFun(head), tail.map(mapFun))

  def filter(filterFun:Int=>Boolean):IntList= if (filterFun(head) == true) 
                                    Cons(head, tail.filterInside(filterFun))
                                else
                                  tail.filterInside(filterFun) 

  def foldLeft(base:Int)(reduceFun:(Int,Int)=>Int):Int= tail.foldLeftInside(reduceFun(base, head))(reduceFun)
  
  def foldRight(base:Int)(reduceFun:(Int,Int)=>Int):Int= reduceFun(head, tail.foldRightInside(base)(reduceFun))
